...
// Types
import type { AttachedFile, TableField, TableFieldAny } from './kz-schema-factory/types.js'
...
import type TableFieldFile from './kz-schema-factory/types-constructors/file.js'
...
class KodzeroToValidnoParser {
  static isAttachedFile(value: unknown): value is AttachedFile {
  ...
  static parseFile(field: TableField<TableFieldFile>) {
    const output: { type: ObjectConstructor | ArrayConstructor; rules?: Record<string, unknown> } = {
      type: Object,
    }
      ...
      if (specsPresent.multiple) {
        ...
        output.rules.custom = (value: unknown, {}) => {
          ...
          const isValid = value.every((file) => {
            if (!KodzeroToValidnoParser.isAttachedFile(file)) {
              return false
            }
        ...
        output.rules.custom = (value: unknown, {}) => {
          ...
          if (!KodzeroToValidnoParser.isAttachedFile(value)) {
            return {
              result: false,
              details: 'Invalid file value',
            }
          ...
          const allowedMimeTypes = field.item.specs.allowedMimeTypes.map((mime) => mime.toLowerCase())
          const mimeTypeAllowed =
            !specsPresent.allowedMimeTypes || allowedMimeTypes.includes(value.mimeType.toLowerCase())
    ...
    for (const field of schemaFiltered) {
      ...
      switch (type) {
        ...
        case 'email':
          ...
          break
        case 'file':
          schema[key] = KodzeroToValidnoParser.parseFile(field as TableField<TableFieldFile>)
          break
        case 'json':
